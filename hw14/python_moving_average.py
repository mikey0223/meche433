...
def plotfft(filename: str, num_points_averaged: int):
    t = [] # column 0
    data1 = [] # column 1
    t_avg = []
    data1_avg = []
    # data2 = [] # column 2
    moving_avg_list = [0]*num_points_averaged
    ...
    with open(filename) as f:
        ...
        for i, row in enumerate(reader):
            ...
            moving_avg_list[i%num_points_averaged] = float(row[1])
            if i >= num_points_averaged-1:
                t_avg.append(float(row[0]))
                data1_avg.append(float(np.average(moving_avg_list)))
            # else:
            #     t_avg.append(0)
            #     data1_avg.append(0)
    ...
    fig.suptitle(filename[:-4] + " " + str(num_points_averaged) + "-points averaged")
    axs[0,0].plot(t,y,'b')
...
plotfft("sigA.csv", 1000)
# input()
plotfft("sigB.csv",1000)
# input()
plotfft("sigC.csv",2000)
# input()
plotfft("sigD.csv",1000)